...
const int MAX_N = 6;
vector<int> strategies(pow(2, MAX_N), 0);
vector<int> child_situation(pow(2, MAX_N), -1);
...
void get_pre_must_win_situation(size_t cur_situation, vector<size_t> & pre_situations) {
        ...
        if (cur_situation_set.find(s) == cur_situation_set.end()) {
            ...
            for (auto i: tmp_set) {
                if (i == 1) {
                    ...
                    if (set_with_s.find(i - 1) != set_with_s.end() && set_with_s.find(i + 1) != set_with_s.end())
                        pre_situation_set.insert(i);
                }
            }
